...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_profile'])) {
    ...
    if ($title && $content) {
        if ($profileModel->addProfile($title, $content)) {
            $_SESSION['message'] = '<div class="alert alert-success">Profil berhasil ditambahkan!</div>';
            redirect('index.php');
        } else {
            $message = '<div class="alert alert-danger">Gagal menambahkan profil.</div>';
        }
    } else {
...
<head>
    ...
    <title>Tambah Profil Baru - Admin</title>
...
<body>
    ...
    <div class="container mt-4">
        <h1 class="mb-4">Tambah Profil Baru</h1>
        <?php echo $message; ?>
        <form action="" method="POST">
            <div class="mb-3">
                <label for="title" class="form-label">Judul Profil (contoh: Tentang Kami, Kontak)</label>
            ...
            <button type="submit" name="add_profile" class="btn btn-primary">Simpan Profil</button>
